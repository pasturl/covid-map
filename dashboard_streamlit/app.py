...
def plot_ts(df, kpi_ts):
    fig = px.line(df, x='yw', y=kpi_ts, labels=dict(yw="Date (year-week)"))
    fig.update_layout(
        width=1000,
        height=500)
    return fig
...
st.subheader('Visualization of evolution of main pandemic KPIs'
...
country = st.sidebar.selectbox('Country', locations)
...
expander_time = st.expander("Dynamic map with time-lapse covid evolution")

# display streamlit map
expander_time.plotly_chart(fig)
my_expander_agg = st.expander("Static map with aggregated KPI")
